...
export const icons = defineCommand({
  ...
  async handler() {
    ...
    if (!existsSync(svg)) {
      log.error('No icons/icon.svg found');
      process.exit(1);
    }
    ...
    const sharpOk = sizes.every((s) => {
      ...
      const r = spawnSync(
        'npx',
        ['sharp-cli', '-i', svg, '-o', out, 'resize', String(s), String(s)],
        {
          stdio: 'pipe',
          shell: false,
        },
    ...
    const pyLines = sizes
      .map(
        (s) =>
          `cairosvg.svg2png(url=sys.argv[1], write_to=sys.argv[${sizes.indexOf(s) + 2}], output_width=${s}, output_height=${s})`,
      )
      .join('\n');
    ...
    const pyArgs = [
      ...
      pyScript,
      svg,
      ...sizes.map((s) => join(process.cwd(), `icons/icon-${s}.png`)),
    ];
