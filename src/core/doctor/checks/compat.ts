...
export const compatCheck: Check = {
  ...
  async run({ cwd }) {
    try {
      ...
      const browsers = (cfg.browsers ?? ['chrome']) as ReadonlyArray<
        'chrome' | 'firefox' | 'edge' | 'safari'
      >;
      const srcDir = resolve(cwd, cfg.build?.srcDir ?? 'src');
      if (!existsSync(srcDir))
        return { name: 'compat', status: 'pass', message: 'No source directory to scan' };
      try {
      ...
      if (total === 0)
        return { name: 'compat', status: 'pass', message: 'No cross-browser compat issues' };
      return {
        name: 'compat',
        status: 'warn',
        message: `${total} compat issue(s) — run \`extforge build\` for details`,
      };
    } catch {
      return { name: 'compat', status: 'info', message: 'Skipped' };
    }
  },
};
