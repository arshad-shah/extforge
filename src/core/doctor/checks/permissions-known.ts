...
const KNOWN = new Set([
  ...
  'enterprise.hardwarePlatformKeys',
  ...
  async run({ cwd }) {
    try {
      ...
      const unknown = perms.filter(
        (p) => !KNOWN.has(p) && !p.startsWith('http') && p !== '<all_urls>',
      );
      if (unknown.length === 0)
        return { name: 'permissions-known', status: 'pass', message: 'Permissions OK' };
      return {
        name: 'permissions-known',
        status: 'warn',
      ...
      return { name: 'permissions-known', status: 'info', message: 'Skipped (config invalid)' };
    }
  },
};
