...
export interface RuntimeFake {
  readonly chrome: {
    id: string;
    onInstalled: {
      addListener(fn: (d: InstalledDetails) => void): void;
      removeListener(fn: (d: InstalledDetails) => void): void;
    };
    onStartup: { addListener(fn: () => void): void; removeListener(fn: () => void): void };
    onMessage: {
      addListener(fn: (m: any, sender: any, send: (r: any) => void) => boolean | void): void;
