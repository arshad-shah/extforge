...
function makeFakeChrome(initial: Record<string, unknown> = {}): {
  chrome: { storage: FakeChromeStorage };
  store: Record<string, unknown>;
} {
  const store: Record<string, unknown> = { ...initial };
  const listeners: Array<
    (c: Record<string, { newValue?: unknown; oldValue?: unknown }>, a: string) => void
  > = [];
  const fakeStorage: FakeChromeStorage = {
    ...
    onChanged: {
      ...
      _fire: (c, a) => {
        for (const l of listeners) l(c, a);
      },
    },
  };
...
function Probe(props: {
  storageKey: string;
  defaultValue: string;
  onState?: (v: unknown, isLoading: boolean) => void;
}): React.ReactElement {
  const { value, setValue, remove, isLoading } = useStorage<string>(
    props.storageKey,
    props.defaultValue,
  );
  props.onState?.(value, isLoading);
  // Stash setters on a deterministic ref for tests to drive.
  (Probe as { _setValue?: (v: string) => Promise<void>; _remove?: () => Promise<void> })._setValue =
    setValue;
  (Probe as { _setValue?: (v: string) => Promise<void>; _remove?: () => Promise<void> })._remove =
    remove;
...
describe('useStorage', () => {
  ...
  it('reflects external chrome.storage.onChanged events into React state', async () => {
    ...
    await act(async () => {
      chrome.storage.onChanged._fire(
        { external: { oldValue: 'initial', newValue: 'changed' } },
        'local',
      );
    });
  ...
  it('unmounting unsubscribes the watch listener (no setState-after-unmount)', async () => {
    ...
    expect(() =>
      chrome.storage.onChanged._fire({ unmount: { oldValue: 'a', newValue: 'b' } }, 'local'),
    ).not.toThrow();
  });
});
