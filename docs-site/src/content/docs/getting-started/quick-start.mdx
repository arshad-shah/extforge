...
Run `extforge dev --open` (or add `dev: { open: true }` to `extforge.config.ts`) and ExtForge launches Chrome with the extension already installed, using a persistent profile under `.extforge/profile/` so logins and devtools layout survive restarts. Pass `--browser firefox --open` to do the same via `web-ext` (run `npm i -D web-ext` first).

Without `--open`, install it by hand:
