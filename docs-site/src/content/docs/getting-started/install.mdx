...
- **Node.js 22.12 or later.** ExtForge uses native ESM, and its CLI dependencies (e.g. `@arshad-shah/clif`) require Node 22.12+. Node 22 LTS is the tested baseline.
...
- Node.js version (✓ if 22.12+, ✗ if below)
...
Run `extforge dev --open` to skip the manual steps below — ExtForge launches the browser with the extension already installed, in a persistent profile under `.extforge/profile/`. See [`dev` config](/reference/config/dev/) to make `--open` the default or point it at a specific binary.
