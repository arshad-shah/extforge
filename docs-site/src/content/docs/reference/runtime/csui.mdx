...
Every manifest-affecting option (`matches`, `excludeMatches`, `runAt`, `allFrames`, `matchAboutBlank`, `world`) is read by that same static scan, so each has to be a **literal** in the `defineCSUI` call — only the key at the top level of the options object counts, so a nested object with the same key name can't shadow it. If you declare one and the scan can't read the value (a variable, a spread, a computed expression), the build warns by name and leaves that option out of the manifest; declare the content script in `extforge.config.ts` when you need a computed value.
...
| `excludeMatches` | `string[]` | Patterns subtracted from `matches`. **Read at build time** — must be a static literal array |
| `runAt` | `'document_start' \| 'document_end' \| 'document_idle'` | Forwarded to manifest entry. Default `document_idle` |
| `allFrames` | `boolean` | Mount in every frame of a matching page. **Read at build time** — must be a `true`/`false` literal. Omitted when unset |
| `matchAboutBlank` | `boolean` | Also inject into `about:blank` frames whose creator matches. **Read at build time** — must be a `true`/`false` literal |
| `world` | `'MAIN' \| 'ISOLATED'` | Execution world. `'MAIN'` has no extension APIs. **Read at build time** — must be a string literal |
...
| Export | Kind | Notes |
|---|---|---|
| `defineCSUI(options, render)` | function | Declare a content-script UI. Default-export it from a `*.csui.tsx` file for auto-discovery. |
| `mountCSUI(descriptor)` | function | Manually mount a descriptor (see above). |
| `CSUIOptions` | interface | `matches`, plus mount/style options passed to `defineCSUI`. |
| `CSUIDescriptor<TRender>` | interface | The object returned by `defineCSUI` and consumed by the runtime / `mountCSUI`. |
| `Renderer` | type | `(root: Element) => void \| (() => void) \| Promise<…>` — your mount function; an optional returned callback runs on unmount. |
