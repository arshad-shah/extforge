...
- **Open CSS pipeline** — Built-in `tailwind` / `vanilla` / `none` presets, plus custom processors (inline `transform` or CLI `command`) and an `onCssTransform` plugin hook so any toolchain plugs in. See the [Styling guide](/guides/styling/).
