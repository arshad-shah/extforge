...
<Aside type="note">
`ctx.addEntry()` and `ctx.emitFile()` are declared in the type but not yet implemented (planned for a future release). Calling either will throw a descriptive error at build time.
</Aside>
