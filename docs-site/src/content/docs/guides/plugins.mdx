...
| `onCssTransform` | Per stylesheet, after the base CSS processor | `(ctx) => string \| void` |
...
## Worked example 1: a CSS processor plugin

`onCssTransform` runs for every stylesheet ExtForge emits, after the base CSS
strategy (`config.css`) has run. Each hook receives the previous step's output
as `ctx.code`, so plugins can chain. Here's a plugin that runs every stylesheet
through [Lightning CSS](https://lightningcss.dev) for autoprefixing and
minification:
...
export default defineConfig({
  plugins: [presetLightning()],
});
```

<Aside type="tip">
For one-off processors you don't need a plugin at all — point `config.css` at a
custom processor object directly. See the [Styling guide](/guides/styling/) for
the full menu of options (presets, CLI commands, inline transforms, and plugins).
</Aside>
...
<Aside type="note">
`ctx.addEntry()` registers a synthetic entry point (bundled into every build,
routed to the ESM or IIFE pass by its `format`), and `ctx.emitFile()` writes a
file into each browser's output directory. Call both from `setup()`; repeated
calls de-duplicate by entry name / output path. Paths passed to `emitFile` that
...
export default {
  plugins: [{
    ...
    apiVersion: 1,
    setup(ctx) {
      ctx.addEntry({ name: 'worker', file: 'src/worker.ts', format: 'esm' });
      ctx.emitFile('build-info.json', JSON.stringify({ builtAt: Date.now() }));
    },
  }],
};
```
