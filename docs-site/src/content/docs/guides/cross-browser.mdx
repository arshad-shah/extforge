...
<Aside type="note">
The compat scan covers entry-point source files. Transitive imports are not yet scanned (planned for v0.4). For now, consolidate API calls in entry-point files or manually suppress with `// extforge-ignore-compat`.
</Aside>
